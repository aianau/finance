...
def main():
    ...
    scenarios = ['rw-lock-vector', 'double-moka', 'rw-lock-arc-vector']
    capacities = [256, 512, 1024, 2048, 4096]
    cache_capacities = [64, 128, 256]
    events_list = [500000]
    threads_list = [6, 10, 12, 16]
    read_write_ratios = [6, 10, 16]
    size_pairs = [(60, 200), (10000, 20000)]
    
    # generate all valid combinations (capacity > cache_capacity)
    # order: capacity -> cache_capacity -> events -> threads -> read_write_ratio -> size_pairs -> scenarios
    all_combinations = []
    for capacity, cache_capacity, events, threads, read_write_ratio, size_pair, scenario in itertools.product(
        capacities, cache_capacities, events_list, threads_list, read_write_ratios, size_pairs, scenarios
    ):
        if capacity > cache_capacity:
            all_combinations.append({
                ...
                'min_size': size_pair[0],
                'max_size': size_pair[1]
            })
    ...
    csv_writer.writerow([
        'Global', 'Cache', 'Events', 'Threads', 'Read/Write Ratio', 'Min Size', 'Max Size', 'Scenario',
    ...
    for combo in all_combinations:
        ...
        print(f"  Size range       : {combo['min_size']} - {combo['max_size']} bytes")
