...
fn run<T>(scenario: &mut T, events: u32, min: u32, max: u32, nr_reads: u32, bimodal: bool) -> (Duration, Duration)
where
    T: Scenario + Send + Sync + 'static,
{
    ...
    for _ in 0..events {
        let event_size = if bimodal {
            use rand::Rng;
            let mut thread_rng = rand::thread_rng();
            
            if thread_rng.gen_ratio(95, 100) {
                // 95% chance
                thread_rng.gen_range(60..=1024)
            } else {
                // 5% chance
                thread_rng.gen_range(10_000..=65_000)
            }
        } else {
            // uniform distribution
            rng.next_int(min, max) as usize
        };
    ...
    T: Scenario + Send + Sync + 'static,
{
    ...
    for _ in 0..args.threads {
        ...
        let bimodal = args.bimodal;
        th.push(thread::spawn(move || {
            let (duration, write_time) = black_box(run(&mut scenario, events, min, max, read_write_ratio, bimodal));
    ...
    println!("=============================================================");
    println!("Distribution mode   : {}", if args.bimodal { "Bimodal (95% small, 5% large)" } else { "Uniform" });
